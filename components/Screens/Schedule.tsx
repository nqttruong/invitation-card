...
const Schedule: React.FC<ScheduleProps> = ({ guestName, onNavigate }) => {
  ...
  return (
    <InvitationCard>
      <p className="text-[#b05d6d] italic text-sm mb-6">Lịch trình chi tiết cho {guestName}</p>
      <div className="font-cursive text-5xl text-[#b05d6d] mb-10">Lịch Trình</div>
      ...
      <div className="space-y-6 text-left max-w-xs mx-auto mb-12">
        {activities.map((item, idx) => (
          <div key={idx} className="flex gap-4 items-start animate-in slide-in-from-left duration-500 delay-[100ms]" style={{ animationDelay: `${idx * 150}ms` }}>
            <div className="font-bold text-[#b05d6d] whitespace-nowrap">{item.time}</div>
            <div className="text-[#5a3e3e] leading-snug">{item.detail}</div>
          </div>
        ))}
      </div>

      <div className="bg-[#fdf2f4] p-4 rounded-xl mb-10 text-xs text-[#b05d6d] italic">
